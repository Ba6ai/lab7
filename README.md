...
#### В статический метод CreateList передаётся число пользователя о количестве элементов в списке. Далее в цикле for добавляются в список случайно сгенерированные числа от 1 до 19. Далее сортировка списка с помощтю метода Sort().
#### В универсальный статический метод ListCopy передаются два списка, <T> позволяет работать с любыми типами данных, работает с типами данных которые поддерживают интерфейс IComparable. Ищем место для элементов второго списка в списке1: если число меньше текущего - вставляем его перед ним и выходим из внутреннего цикла. Если прошёл весь первый список и вставка не произошла, значит число больше всех имеющихся и вставляется в конец списка.
#### В универсальный статический метод Print передаётся список. В цикле for проходит по всей длинне списка и выводит элементы через пробел.
