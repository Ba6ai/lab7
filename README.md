...
#### В статический метод ListCopy передаются два списка. Ищем место для элементов второго списка в списке1: если число меньше текущего - вставляем его перед ним и выходим из внутреннего цикла. Если прошёл весь первый список и вставка не произошла, значит число больше всех имеющихся и вставляется в конец списка.
#### В статический метод Print передаётся список. В цикле for проходит по всей длинне списка и выводит элементы через пробел.
